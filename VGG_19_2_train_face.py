...
import time
import VGG_19_2_inference_random_normal as VGG

original_photo_path = './VGG for photo/my_face_224'
modified_photo_path = './VGG for photo/other_face_224'
size = 224
test_size = 50
...
test_x_sized = test_x[0:test_size]
test_y_sized = test_y[0:test_size]
...
def cnnTrain() : 
	...
	cross_entropy = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits = out.fc8, labels = y_))
	
	train_step = tf.train.AdamOptimizer(0.0001).minimize(cross_entropy)
#    train_step = tf.train.AdamOptimizer(0.0001).minimize(cross_entropy)
	# to compare the label is equal or not, and get average accuracy, tf.cast to convert. 
	
	accuracy = tf.reduce_mean(tf.cast(tf.equal(tf.argmax(out.fc8, 1), tf.argmax(y_, 1)), tf.float32))
		...
		for n in range(20) :  # iterated 20 times. 
			...
			for i in range(num_batch) : 
				...
				if (n*num_batch+i) % 10 == 0 : 
					# check the accuracy 
		#			acc = accuracy.eval({x: test_x_sized, y_: test_y_sized, keep_prob_5 : 1.0})
					temp_fc8_train = sess.run(out.fc8, {x:batch_x, y_:batch_y, keep_prob_5 : 0.5})
					temp_loss = sess.run(cross_entropy, {x:batch_x, y_:batch_y, keep_prob_5 : 0.5})
					temp_fc8_test = sess.run(out.fc8, feed_dict = {x: test_x_sized, y_: test_y_sized, keep_prob_5 : 1.0})
					acc = sess.run(accuracy, feed_dict = {x: test_x_sized, y_: test_y_sized, keep_prob_5 : 1.0})
