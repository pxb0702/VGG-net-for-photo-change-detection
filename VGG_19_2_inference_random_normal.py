...
# 2018-4-11 : this program is to use Random_Normal as the parameter initializer
#             to see if the "all 0" issue is related with initializer or not
...
class VGG19_2_inference() :
    ...
    def conv_op(self, input_op, name, kh, kw, n_out, dh, dw, p) : 
        ...
        with tf.name_scope(name) as scope : 
            para_shape = [kh, kw, n_in,n_out]
            init = tf.random_normal(para_shape, stddev = 0.01)
...
#                dtype = tf.float32, initializer= init) 
    ...
    def fc_op (self, input_op, name, n_out, p) : 
        ...
        with tf.name_scope(name) as scope : 
            para_shape = [n_in,n_out]
            init = tf.random_normal(para_shape, stddev = 0.01)
            kernel = tf.Variable(init)
            
#            kernel = tf.get_variable(scope+"w", 
#                shape = para_shape, 
#                dtype = tf.float32, initializer= tf.random_normal(stddev = 0.01)) 
